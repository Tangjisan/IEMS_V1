...
/***********************************************
 LCD重要参数集
************************************************/
LCD_Dev_Structure lcddev;   //管理LCD重要参数,默认为竖屏
...
/***********************************************
 宏函数定义
************************************************/
#define LCD_WR_REG(regval)  LCD->LCD_REG = regval   //LCD仅写寄存器
#define LCD_WR_DATA(data)   LCD->LCD_RAM = data     //LCD写数据
#define LCD_RD_DATA()       LCD->LCD_RAM            //LCD读数据
...
/***********************************************
 *ILI93xx读出的数据转为RGB格式
 *函数名：u16 LCD_BGR2RGB(uint16_t c)
 *参数：c-GBR格式的颜色值
 *返回值：RGB格式的颜色值 
 *时间：2018-08-29
 *备注：基于正点原子STM32F1精英版
...
/***********************************************
 *在指定区域内填充指定颜色块
 *函数名：void LCD_Color_Fill(uint16_t sx,uint16_t sy,uint16_t ex,uint16_t ey,uint16_t *color)
 *参数：(sx,sy),(ex,ey)-填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)
        color-要填充的颜色
 *返回值：无 
 *时间：2018-08-29
