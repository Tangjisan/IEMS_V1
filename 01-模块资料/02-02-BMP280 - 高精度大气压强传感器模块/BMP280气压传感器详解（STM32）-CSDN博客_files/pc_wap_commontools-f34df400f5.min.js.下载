...
$(function () {
  if (!isOpenSourceBlog) {
    ...
    if (!isApp) {
      if (isNewGitCodeAB) {
        // gitcode 高亮词实验组
        var content = $("#article_content").clone().find("pre, div.mermaid, div.toc, a.has-card, div.article-questions-container").remove().end().text();
          ...
          success: function success(data) {
            if (data.highlights && data.highlights.length > 0) {
              data.highlights.forEach(function (item) {
                gitcodeMap.set(item.keyword, item.link);
              });
